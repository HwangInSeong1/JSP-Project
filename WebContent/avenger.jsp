    ...
    <body>
        ...
        <th colspan=2 style="border:0px solid white;"> <a href="write.jsp" target=if1><input type=button value="작성하기"></a></th>
            ...
            <%
            	...
            	ArrayList<Mvr> list02 = mvrDAO.getList02();
            	for(int i = 0; i < list02.size(); i++) {
            %>
            <tr>
                <th><%= list02.get(i).getR_NUM() %></th> 
                <th><a href="view.jsp?R_NUM=<%= list02.get(i).getR_NUM() %>"><%= list02.get(i).getR_Title() %></a></th> 
                <th><%= list02.get(i).getUser_ID() %></th> 
                <th><%= list02.get(i).getR_DATE().substring(0, 11) + list02.get(i).getR_DATE().substring(11, 13) + "시" + list02.get(i).getR_DATE().substring(14, 16) + "분" %></th>
            </tr>
			<%
            	}
            %>
