    ...
    %>
        ...
        <th colspan=2 style="border:0px solid white;"> <a href="write.jsp" target=if1><input type=button value="작성하기"></a></th>
            ...
            <%
            	...
            	ArrayList<Mvr> list = mvrDAO.getList(pageNumber);
            	for(int i = 0; i < list.size(); i++) {
            %>
            <tr>
                <th><%= list.get(i).getR_NUM() %></th> 
                <th><a href="view.jsp?R_NUM=<%= list.get(i).getR_NUM() %>"><%= list.get(i).getR_Title() %></a></th> 
                <th><%= list.get(i).getUser_ID() %></th> 
                <th><%= list.get(i).getR_DATE().substring(0, 11) + list.get(i).getR_DATE().substring(11, 13) + "시" + list.get(i).getR_DATE().substring(14, 16) + "분" %></th>
            </tr>
			<%
            	}
            %>
        	...
        	if(pageNumber != 1) {
       		%>
        	<a href="Review.jsp?pageNumber=<%=pageNumber - 1%>"><input type=button value="이전 페이지"></a>
        	<%
        	} if(mvrDAO.nextPage(pageNumber + 1)) {
        	%>
        	<a href="Review.jsp?pageNumber=<%=pageNumber + 1%>"><input type=button value="다음 페이지"></a>
        	<%
        	}	
        	%>
